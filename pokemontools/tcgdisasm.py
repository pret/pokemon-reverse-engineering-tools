                ...
                if   op_code_type == 0 and rom[offset] == op_code_byte:
                    ...
                    if current_byte == 0xc3 or current_byte in relative_unconditional_jumps:
                        ...
                        if not has_outstanding_labels(byte_labels) or all_outstanding_labels_are_reverse(byte_labels, offset):
                            ...
                            is_data = False
                            break
                elif op_code_type == 4 and rom[offset] == op_code_byte:
                    oplen = len(op_code[0])
                    opstr = copy(op_code[0])
                    if op_code_byte == 0xef:
                        xes = op_code[0].count("x")
                        for x in range(0, xes):
                            temp_bank = rom[offset + 1]
                            insertion = "$" + hex(temp_bank)[2:]
                            temp_opstr = opstr[:opstr.find("x")].lower() + insertion + opstr[opstr.find("x")+1:].lower()
                    ...
                    for x in range(0, qes):
                        ...
                        if op_code_byte == 0xef:
                            if result != None:
                                insertion = result
                                opstr = opstr[:opstr.find("x, ?")].lower() + insertion + opstr[opstr.find("x, ?")+4:].lower()
                            else:
                                opstr = temp_opstr[:temp_opstr.find("?")].lower() + insertion + temp_opstr[temp_opstr.find("?")+1:].lower()
                                opstr = "farcallx" + opstr[7:]
                        else:
                            if result != None:
                                insertion = result
                            opstr = opstr[:opstr.find("?")].lower() + insertion + opstr[opstr.find("?")+1:].lower()
...
if __name__ == "__main__":
    ...
    if addr / 0x4000 == 0:
        if addr < 0x1000:
            output = "Func_0{:02x}: ; 0{:02x} ({:0x}:0{:02x})\n".format(addr, addr, addr / 0x4000, addr)
        else:
            output = "Func_{:02x}: ; {:02x} ({:0x}:{:02x})\n".format(addr, addr, addr / 0x4000, addr % 0x4000)
    else:
